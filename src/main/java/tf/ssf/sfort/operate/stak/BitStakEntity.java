...
public class BitStakEntity extends BlockEntity {
    ...
    public void applyToAllConnectedColorTubes(Consumer<ColorTubeEntity> process) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (e instanceof ColorTubeEntity && ((ColorTubeEntity) e).sides[dir.getOpposite().ordinal()] == TubeConnectTypes.ALL) {
        ...
        do {
            for (ColorTubeEntity e : reader) {
                for (int i = 0, size = e.sides.length; i < size; i++) {
                    TubeConnectTypes side = e.sides[i];
                    if (side != TubeConnectTypes.ALL) continue;
                    Direction dir = Direction.values()[i];
                    BlockEntity neighbour = world.getBlockEntity(mut.set(e.getPos()).move(dir));
                    if (neighbour instanceof ColorTubeEntity && ((ColorTubeEntity) neighbour).sides[dir.getOpposite().ordinal()] == TubeConnectTypes.ALL) {
